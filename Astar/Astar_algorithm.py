...
class Astar:
    ...
    def planning(self, start_x, start_y, goal_x, goal_y,max_itertime):
        ...
        output:
            path:
        """
        start_node=self.Node(start_x,start_y,-1)
        goal_node=self.Node(goal_x,goal_y,-1)
        ...
        for i in range(max_itertime):
            ...
            if current_node.x == goal_node.x and current_node.y == goal_node.y:
                print("Find goal with cost",current_node.f,"after ",i,"th iteration")
...
if __name__ == '__main__': 
    ...
    screen.fill((0,0,0))
    running=True
    filepath="./Dijkstra/obstacle2.png"
    ...
    astar=Astar(0,0,screen,img_matrix)
    idx=0
    ...
    while running:
        if idx<1:
            ...
            pygame.draw.circle(screen,(0,0,255),start,2.5)
            pygame.display.update()
            path=astar.planning(start[0],start[1],goal[0],goal[1],1000000)
            idx=1   
        whether_quit()